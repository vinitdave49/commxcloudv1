...
//var notifyTyping = function(){}
  ...
  function fetchAccessToken(username, handler) {
    $.post('/web/app_dev.php/getchattoken?identity=ashwin&device=browser', {
      identity: username,
      device: 'browser'
    }, function(data) {
      handler(data);
  ...
  function connectMessagingClient(tokenResponse) {
    ...
    tc.messagingClient.on('channelAdded', $.throttle(100,tc.loadChannelList));
    tc.messagingClient.on('channelRemoved', $.throttle(100,tc.loadChannelList));
  ...
  tc.joinGeneralChannel = function() {
    ...
    if (!tc.generalChannel) {
      ...
      tc.messagingClient.createChannel({
        ...
        console.log('Created general channel' + channel.sid);
        tc.generalChannel = channel;
        tc.loadChannelList(tc.joinGeneralChannel);
      });
    }
    else {
      console.log('Found general channel:' + tc.generalChannel.sid);
  ...
  tc.addMessageToList = function(message) {
    ...
    rowDiv.loadTemplate($('#message-template'), {
      ...
      date: DateFormatter.format(now, "Y/m/d H:i:s"),
      body: message.body
    });
